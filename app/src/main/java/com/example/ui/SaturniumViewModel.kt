...
class SaturniumViewModel(
    ...
    fun toggleWorkDayForDay(config: CycleDayConfig) {
        ...
        // Auto-disable alarm if converting to off day or vice versa
        val alarmOn = if (isWorkNow) true else config.alarmEnabled
        val newName = if (isWorkNow) "Рабочая смена" else "Выходной"
    ...
    /**
     * Force dismiss the alarm in simulator / real state
     */
    fun dismissAlarm() {
    ...
    /**
     * Simulate an immediate ring for visual verification / testing
     */
    fun triggerInstantSimulation() {
        ...
        Log.d(TAG, "Simulated ringing state triggered!")
    }
